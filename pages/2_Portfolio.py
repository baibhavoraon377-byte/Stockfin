...
if st.session_state.portfolio['holdings']:
    ...
    st.markdown('<div style="font-size:.75rem;color:var(--text-secondary);margin-bottom:.8rem;">Double-click the <b>Quantity</b> cells to edit your holdings inline.</div>', unsafe_allow_html=True)
    ...
    if not df_grid.empty:
        ...
        gb.configure_column("Current Price", valueFormatter="data['Current Price'] ? '$' + Number(data['Current Price']).toFixed(2) : ''", width=120)
        gb.configure_column("Invested", valueFormatter="data['Invested'] ? '$' + Number(data['Invested']).toFixed(0) : ''", width=120)
        ...
        class AllocRenderer {
            init(params) {
                ...
                this.eGui.innerHTML = 
                   '<div style="width: 45px; text-align: right; margin-right: 10px;">' + Number(val).toFixed(1) + '%</div>' +
                   '<div style="flex-grow: 1; height: 6px; background: rgba(255,255,255,0.06); border-radius: 10px; overflow: hidden;">' +
                   '<div style="height: 100%; width: ' + pct + '%; background: linear-gradient(90deg, #4f8fff, #7c6ff7); border-radius: 10px;"></div>' +
                   '</div>';
            }
            getGui() {
                return this.eGui;
        ...
        """)
        gb.configure_column("Alloc %", cellRenderer=alloc_renderer, width=180)

        gb.configure_grid_options(rowHeight=45, domLayout='autoHeight')
